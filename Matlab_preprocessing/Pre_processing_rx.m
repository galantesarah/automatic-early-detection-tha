...
folder_path = fullfile(directory,'/PRIN_mancanti_rgb/'); %cartella specifica dove ci sono dati che ti interesaanto
...
for i = 1:numel(listdir)
    ...
    saving_dir = fullfile(directory,'/PRIN_mancanti_proc/');
    ...
    saving_dir = fullfile(directory,'/PRIN_mancanti_proc/'); %directory di salvataggio
...
for i = 1:numel(listdir)
    ...
    im = mat2gray(im);
    medie_totali = mean2(im);
    saving_dir = fullfile(directory, '/PRIN_mancanti_proc_2/');
...
image_name = 'O2-A-101-1m AP.jpg'; % Nome dell'immagine che desideri processare
